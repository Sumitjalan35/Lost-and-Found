...
export default function FoundItemForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (image) {
        const imageRef = ref(storage, `found-items/${Date.now()}-${image.name}`);
      ...
      toast.success('Found item reported successfully! Email notification sent to college community.');
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white dark:bg-gray-800 shadow-lg rounded-lg">
      <h2 className="text-2xl font-bold mb-6 text-center text-gray-800 dark:text-white">Report Found Item</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              required
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
              placeholder="e.g., Lost & Found Office, My Dorm Room"
            />
          </div>
        </div>
        ...
        <div>
          ...
          <div
            ...
            className={`border-2 border-dashed rounded-lg p-6 text-center cursor-pointer transition-colors ${
              isDragActive ? 'border-green-500 bg-green-50 dark:border-green-600 dark:bg-green-900' : 'border-gray-300 dark:border-gray-600 hover:border-gray-400 dark:hover:border-gray-500'
            }`}
          >
            <input {...getInputProps()} />
            {imagePreview ? (
